...
import { 
  Mail, Phone, ArrowRight, Loader2, Key, 
  Fingerprint, AlertCircle, RefreshCw
} from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleRequestOtp = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorMsg = typeof data.error === "object" ? data.error.message : data.error;
        setError(errorMsg || "Failed to send code");
        return;
      }
  ...
  const handleVerifyOtp = async (code: string) => {
    ...
    try {
      const res = await fetch("/api/auth/signin/otp", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ identifier, identifierType, code }),
      });
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground text-sm">
                Sign in to continue to CharacterDNA
              </p>
            </div>
            ...
            <form onSubmit={loginMethod === "password" ? handlePasswordLogin : (e) => { e.preventDefault(); handleRequestOtp(); }} className="space-y-4">
              {identifierType === "email" ? (
